...
(function (global) {
  ...
  const Models = {
    ...
    listBranches() {
      return DB.exec("SELECT * FROM branches ORDER BY name");
    },
    createBranch(data) {
      DB.run(
        `INSERT INTO branches (name, address, phone, logo_data_url, slogan, tax_rate, round_to, license_to, created_at)
         VALUES (?,?,?,?,?,?,?,?,?)`,
        [data.name, data.address || '', data.phone || '', data.logo_data_url || null,
         data.slogan || '', data.tax_rate || 0, data.round_to || 1000, data.license_to || '', Date.now()]
      );
      return DB.lastInsertId();
    },
    deleteBranch(branchId) {
      // Xoá toàn bộ dữ liệu chi nhánh (orders, items, products, categories, ingredients, users, inventory_tx)
      DB.run("DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE branch_id=?)", [branchId]);
      DB.run("DELETE FROM orders WHERE branch_id=?", [branchId]);
      DB.run("DELETE FROM product_variant_groups WHERE product_id IN (SELECT id FROM products WHERE branch_id=?)", [branchId]);
      DB.run("DELETE FROM products WHERE branch_id=?", [branchId]);
      DB.run("DELETE FROM categories WHERE branch_id=?", [branchId]);
      DB.run("DELETE FROM inventory_transactions WHERE branch_id=?", [branchId]);
      DB.run("DELETE FROM ingredients WHERE branch_id=?", [branchId]);
      DB.run("DELETE FROM recipes WHERE product_id IN (SELECT id FROM products WHERE branch_id=?)", [branchId]);
      DB.run("DELETE FROM users WHERE branch_id=?", [branchId]);
