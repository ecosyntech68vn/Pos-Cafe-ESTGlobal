...
(function (global) {
  ...
  function runMigrations() {
    ...
    // ---- v4 (2026-06-13): PERF index cho freeze fix ----
    if (cur < 4) {
      console.log('[migrate] v3 → v4 (PERF index)');
      try { db.exec('CREATE INDEX IF NOT EXISTS idx_orders_kitchen ON orders(branch_id, kitchen_status, created_at)'); }
      catch (e) { console.warn('[migrate] idx_orders_kitchen', e.message); }
      try { db.exec('CREATE INDEX IF NOT EXISTS idx_order_items_order ON order_items(order_id)'); }
      catch (e) { console.warn('[migrate] idx_order_items_order', e.message); }
      try { db.exec('CREATE INDEX IF NOT EXISTS idx_orders_branch_created ON orders(branch_id, created_at)'); }
      catch (e) { console.warn('[migrate] idx_orders_branch_created', e.message); }
      recordVersion(4);
    }
  }
  ...
  async function seedIfEmpty() {
    ...
    try {
      ...
      // Icon Win10-safe (🧋/🫐 là Emoji 13 — Windows 10 không có font, hiỆ�7n ô v{Ổng)
      ...
      menuRows.forEach(row => {
        ...
        if (!p) {
          pid++;
          p = { id: pid, base_code: baseCode, name: row.item_code, category_sub: row.category_sub, sizes: {}, single_price: null };
      ...
      // 2026-06-10: chuẩn hoá �àn vị kho về gram/ml (kg→gram ×1000, lít.�
